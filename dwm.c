...
static char *my_getdwmpath(void);
...
static void my_restart(const Arg *arg);
...
// https://dwm.suckless.org/patches/selfrestart/
void
my_restart(const Arg *arg)
{
    char *const argv[] = { my_getdwmpath(), NULL };
...
my_setkblayout(const Arg *args)
{
	...
	if (fork() == 0) {
		...
		execvp(((char **)kb->kbmap)[0], (char **)kb->kbmap);

		kbidx = oldidx;
		kb = oldkb;;
		die("dwm: execvp '%s' failed:", ((char **)kb->kbmap)[0]);
	}
}
